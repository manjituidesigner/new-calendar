...
function renderStats() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // Determine worked minutes for this day
    let workedMinutes = 0
...
function updateSessionBar() {
  ...
  function refresh() {
    ...
    const totalMinutes = Math.max(0, Math.floor((effectiveEnd - startMs) / 60000))
    const breakMinutes = computeBreakMinutes(info)
    ...
    if (sessionWorkedEl) {
      ...
      sessionWorkedEl.textContent = `Worked: ${h}h ${m}m`
    }
    if (sessionBreakEl) {
      sessionBreakEl.textContent = `Break: ${breakMinutes}m`
    }

    const productivity = totalMinutes > 0 ? Math.round((workMinutes / totalMinutes) * 100) : 0
  ...
  if (!info.outTime) {
    sessionIntervalId = setInterval(refresh, 30000)
  }
