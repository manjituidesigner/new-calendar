...
const statTodayWorkingHours = document.getElementById("statTodayWorkingHours")
...
function renderStats() {
  ...
  // todayMinutes: today portion of actualMinutes
  let todayMinutes = 0
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    actualMinutes += actualWorked

    // Track today's minutes separately (timer-based only)
    if (key === todayKey) {
      todayMinutes = actualWorked
    }
  }
  ...
  if (plannedMinutes > 0) {
    ...
    // Fallback to only spent hours when no schedule defined
    statWorkingHours.textContent = `${hours}h ${mins}m`
  }

  if (statTodayWorkingHours) {
    const tHours = Math.floor(todayMinutes / 60)
    const tMins = todayMinutes % 60
    statTodayWorkingHours.textContent = `${tHours}h ${tMins}m`
  }
